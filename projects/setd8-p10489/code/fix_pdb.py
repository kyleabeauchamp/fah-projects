...
def fix(pdbid, missing_residues, padding=PADDING, mutations=None):
    fixer = pdbfixer.PDBFixer(filename="./pdbs/%s.pdb" % pdbid)
...
t = md.load("./pdbs/setd8_initial.pdb")
t = t.atom_slice(t.top.select("chainid 0 and protein"))
t.save("./pdbs/setd8.pdb")

prepend = {(0, 0):["ARG"]}
mutations=(["SER-343-CYS"], "A")
fix("setd8", prepend, mutations=mutations)


n_atoms = 44018  # Want same number of atoms as last project.  NOTE: this relies on the fact that waters are placed last.

t = md.load("./pdbs/setd8_box.pdb")
t = t.atom_slice(np.arange(n_atoms))
t.save("./pdbs/setd8_box.pdb")
